...
namespace CustomActivatableEquipment.DamageHelpers {
  ...
  public static class DamageHelper {
    ...
    internal static void Reset() {
      Log.Debug?.Write($"DamageHelper Reset - new Mission\n");
    ...
    internal static void newTurnFor(AbstractActor actor) {
      Log.Debug?.Write($"new Turn Activation for {actor.Nickname} - {actor.DisplayName} - {actor.GUID}\n");
      attacker = actor;
      if (actor is Mech) {
    ...
    internal static void completedTurnFor(AbstractActor instance) {
      if (attacker != null) {
        Log.Debug?.Write($"completed Turn Activation for {instance.Nickname} - {instance.DisplayName} - {instance.GUID}\n");
        foreach (AbstractActor actor in activationVictims) {
          Log.Debug?.Write($"{instance.DisplayName}|{instance.Nickname}|{instance.GUID} damaged during turn.\n");
    ...
    internal static float MaxArmorForLocation(Mech mech, int Location) {
      if (mech != null) {
        ...
        if (stat == null) {
          Log.Debug?.Write($"Can't get armor stat  { mech.DisplayName } location:{ Location.ToString()}\n");
          return 0;
        }
    ...
    internal static float MaxStructureForLocation(Mech mech, int Location) {
      if (mech != null) {
        ...
        if (stat == null) {
          Log.Debug?.Write($"Can't get structure stat  { mech.DisplayName } location:{ Location.ToString()}\n");
          return 0;
        }
    ...
    internal static void BatchHeatDamage(Mech instance, int amt) {
      if (instance != null && amt > 0) {
        ...
        if (!activationVictims.Contains(instance)) {
          ...
          Log.Debug?.Write($"{instance.DisplayName}|{instance.Nickname}|{instance.GUID} added to victims [heat]\n");
        }
      }
    }
    ...
    internal static void BatchDamage(Mech instance, float damageAmount, float directStructureDamage) {
      ...
      if (instance != null) {
        ...
        if (!activationVictims.Contains(instance)) {
          activationVictims.Add(instance);
          Log.Debug?.Write($"{instance.DisplayName}|{instance.Nickname}|{instance.GUID} added to victims\n");
        }
      }
    }
  }
